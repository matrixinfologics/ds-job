jQuery(document).ready(function () {
    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        jQuery(document).ready(function () {
            // Get the value of the occupation parameter from the URL
            const urlParams = new URLSearchParams(window.location.search);
            const occupationValue = urlParams.get('occupation%5B%5D');
        
            // Check if the decoded occupation is Physician (MD/DO)
            if (decodeURIComponent(occupationValue) === 'Physician (MD/DO)') {
                // Show data when Physician (MD/DO) exists in the URL
                console.log('Show Physician (MD/DO) data here.');
            } else {
                // Show default data when page loads
                if (i === 4) {
                    html += '<tr class="desktopSign cstom1 mobile-apps-1">Test</tr>';
                }
                if (i === 10) {
                    html += '<tr class="desktopSign cstom2 mobile-apps download-app">Test</tr>';
                }
            }
        });        
        }
