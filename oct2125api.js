    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        function updateDatalist() {
            let specialties_html = "";
            for (let j = 0; j < result.specialties.length; j++) {
                specialties_html += `<option value="${result.specialties[j]}">${result.specialties[j]}</option>`;
            }
            jQuery('#lists_speciality').empty().append(specialties_html);
        }
        jQuery('.api-occupation').on('click', function() {
