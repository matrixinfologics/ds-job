    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        let specialty_names_str = result.jobs[i].specialty_names;
          ...
          jQuery(document).on("click",".api-occupation",function(){
            let specialty_names = '';
              for(let k = 0; k < specialty_names_arr.length; k++){
                if( k <= 2){
                  specialty_names += `<label><input type="radio" class="api-speciality" id="${specialty_names_arr[k]}" name="specialty_radio" value="${specialty_names_arr[k]}"></input> ${specialty_names_arr[k]}</label>`;
                }
              }
          });

          jQuery('.listSpeciality').after(`<datalist id="lists_speciality">${specialty_names}</datalist>`);
          ...
          var hotJobElement = hotJob === true ? '<img src="https://uploads-ssl.webflow.com/5abebcf1b3e0cb4bb9718bba/652d3b92927905d052de6d4e_Hot%20job.svg"><span>Hot Job</span>' : '';
