   ...
   //Search jobs
    jQuery(document).on("click", "datalist input", function () {
        ...
        if(jQuery(this).hasClass('api-occupation')){
            ...
            r = new URL("https://www.directshifts.com/open-jobs-8-jan");
    ...
    jQuery("#clear-all-button").on("click", function () {
        ...
        let a = new URL("https://www.directshifts.com/open-jobs-8-jan");
...
var getUrlParameterLoad = function(param) {
    ...
    for (var i = 0; i < url.length; i++) {
        ...
        if (paramPair[0] === param) {
            if (paramPair.length === 2) {
                // Replace '0' with an empty string in the parameter value
                return paramPair[1].replace(/state%5B0%5D=([^&]*)/, 'state%5B%5D=$1').replace(/occupation%5B0%5D=([^&]*)/, 'occupation%5B%5D=$1');
            } else {
                // Replace '0' with an empty string in each element of the array
                return paramPair.slice(1).map(function(value) {
                    return value.replace(/state%5B0%5D=([^&]*)/, 'state%5B%5D=$1').replace(/occupation%5B0%5D=([^&]*)/, 'occupation%5B%5D=$1');
                });
            }
        }
    }
