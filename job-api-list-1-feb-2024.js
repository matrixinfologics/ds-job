    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        if(result.jobs.length <= 3){ jQuery('.sidebtmlft').addClass('notFoundResult'); }
            ...
            function getParameterByNameGlobalNf(name) {
                name = name.replace(/[\[\]]/g, "\\$&");
                var regex = new RegExp("[?&]" + name + "(=([^&#]*)|&|#|$)"),
            ...
            var utmSourceG = getParameterByNameGlobalNf('utm_source');
            var utmMediumG = getParameterByNameGlobalNf('utm_medium');
            var utmCampaignG = getParameterByNameGlobalNf('utm_campaign');
            var utmContentG = getParameterByNameGlobalNf('utm_content');
            var getUtmConG;
            ...
            html = '<div class="nrfound"><h2>We may have job openings for you, even if it\'s not yet listed!</h2><p>Our job listings are constantly evolving. Connect with us to uncover potential job openings tailored to your needs. We\'re here to help you find the right job for you. </p><a class="utm-btn-val-nf" href="https://www.directshifts.com/speak-to-a-recruiter">Connect with a clinician partner</a></div>';
