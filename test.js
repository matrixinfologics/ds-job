    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        for(let j = 0; j < result.states.length; j++){
            states_html += `<option label="${result.states[j].name}" value="${result.states[j].code}"></option>`;
        }
        jQuery('.api-state').after(`<datalist id="lists_state">${states_html}</datalist>`);
        ...
        jQuery('.api-city').after(`<datalist id="lists_city">${cities_html}</datalist>`);

        // specialties
        let specialties_html = "";
        for(let j = 0; j < result.specialties.length; j++){
            specialties_html += `<option value="${result.specialties[j]}"></option>`;
        }
        jQuery('.api-speciality').after(`<datalist id="lists_speciality">${specialties_html}</datalist>`);
        ...
        for(let j = 0; j < uniquePracticeTypes.length; j++){
          practice_type_html += `<option label="${uniquePracticeTypes[j]}" value="${uniquePracticeTypes[j]}"></option>`;
        }
        jQuery('.api-practice-type').after(`<datalist id="lists_practice_type">${practice_type_html}</datalist>`);
        ...
        for(let j = 0; j < uniqueCategories.length; j++){
          categories_html += `<option label="${uniqueCategories[j]}" value="${uniqueCategories[j]}"></option>`;
        }
        jQuery('.api-category').after(`<datalist id="lists_category">${categories_html}</datalist>`);
       ...
       let uniqueOccupations = ['Physician (MD/DO)','Physician Assistant (PA)','Advanced Practice Registered Nurse (APRN, NP, CNS, Midwife, etc.)','Certified Registered Nurse Anesthesist (CRNA)','NurseMidwife','Nurse Specialist/CNS','Registered Nurse (RN)','Licensed Practical Nurse/Licensed Vocational Nurse (LVN, LPN)','Certified Nursing Assistant (CNA)','Patient Care Technician (PCT)','Respiratory, Rehabilitative & Developmental Therapists (OT, PT, etc.)','Technologist (Sonography, Radiology, etc.)','Technician (Radiologic, Cardiovascular, Histo, etc.)','Medical Assistants (CMA, RMA)','Dental Services (DDS, Technician, Hygienist, etc.)','Behavioral Health & Social Work Services (Counselors, Social Workers, etc.)','Dietitians and Nutritionists','Pharmacist','Healthcare Administration'];
        ...
        for(let j = 0; j < uniqueOccupations.length; j++){
          occupations_html += `<option label="${uniqueOccupations[j]}" value="${uniqueOccupations[j]}"></option>`;
        }
        jQuery('.api-occupation').after(`<datalist id="lists_occupation">${occupations_html}</datalist>`);
...
//   Search jobs
jQuery('.api-button').on('click', function(){
    let state = jQuery('.api-state').val();
    let city = jQuery('.api-city').val();
    let occupation = jQuery('.api-occupation').val();
    let specialty = jQuery('.api-speciality').val();
    let category = jQuery('.api-category').val();
    let practice_type = jQuery('.api-practice-type').val();
    
    const open_jobs_url = new URL('https://www.directshifts.com/job-listing-copy-aug-24');
    
    if(state){
        open_jobs_url.searchParams.set('state', state);
    }
    if(city){
        open_jobs_url.searchParams.set('city', city);
    }
    if(occupation){
        open_jobs_url.searchParams.set('occupation', occupation);
    }
    if(specialty){
        open_jobs_url.searchParams.set('specialty', specialty);
    }
    if(category){
        open_jobs_url.searchParams.set('category', category);
    }
    if(practice_type){
        open_jobs_url.searchParams.set('practice_type', practice_type);
    }
    window.history.pushState(null, '', open_jobs_url.toString());
...
jQuery(document).on('click', "span.pgn-func", function() {
  ...
  let page = jQuery(this).attr('data-value');
