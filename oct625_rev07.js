...
//   Search jobs
    ...
    jQuery(document).on("click", "datalist input", function () {
        ...
        if (t!=null && t.length) r.searchParams.set("state[]", t.join(","));
        if (s!=null && s.length) r.searchParams.set("city[]", s.join(","));
        if (i!=null && i.length) r.searchParams.set("occupation[]", i.join(","));
        if (n!=null && n.length) r.searchParams.set("specialty[]", n.join(","));
        if (l!=null && l.length) r.searchParams.set("category[]", l.join(","));
        if (c!=null && c.length) r.searchParams.set("practice_type[]", c.join(","));
