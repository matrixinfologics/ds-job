...
//   Call function when page loads
  ...
  if(queryString){
    ...
    if(occupation != false){
        occupation_ar = occupation.split(",");.map(function(value){ return value.replace("+"," "); })
    }
    
    var specialty = getUrlParameter("specialty[]");
    if(specialty != false){
        specialty_ar = specialty.split(",").map(function(value){ return value.replace("+"," "); });
    }

    var state = getUrlParameter("state[]");
    if(state != false){
        state_ar = state.split(",");
        state_ar = state_ar.map(function(value){ return value.replace("+"," "); })
    }
    var practice_type = getUrlParameter("practice_type[]");
    if(practice_type != false){
        practice_type_ar = practice_type.split(",");
        practice_type_ar = practice_type_ar.map(function(value){ return value.replace("+"," "); })
    }

    apiCall({ state: state_ar, occupation: occupation_ar, specialty: specialty_ar, practice_type: practice_type_ar });
