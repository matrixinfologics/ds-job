jQuery(document).ready(function () {
    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        // specialties
        // var occupation = $('#lists_occupation input');
        // if (occupation) {
        //     occupation.change(function() {
        //         jQuery('#lists_speciality').remove();
        ...
        //jQuery('.listSpeciality').empty().append(`<datalist id="lists_speciality">${specialties_html}</datalist>`);
...
//   Search jobs
    ...
    //     let a = jQuery('input[type="checkbox"]:checked').length;
    ...
    //         r = new URL("https://www.directshifts.com/a-job-listing-copy-2023-oct-19");
    ...
    jQuery(document).on("click", "datalist input", function () {
        ...
        if (typeof t !== undefined && t.length) r.searchParams.set("state[]", t.join(","));
        if (typeof s !== undefined && s.length) r.searchParams.set("city[]", s.join(","));
        if (typeof i !== undefined && i.length) r.searchParams.set("occupation[]", i.join(","));
        if (typeof n !== undefined && n.length) r.searchParams.set("specialty[]", n.join(","));
        if (typeof l !== undefined && l.length) r.searchParams.set("category[]", l.join(","));
        if (typeof c !== undefined && c.length) r.searchParams.set("practice_type[]", c.join(","));
